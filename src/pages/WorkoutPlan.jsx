...
import { Sparkles, LayoutList, History, Loader2, Clock, CheckCircle2, ChevronRight, Share2 } from 'lucide-react';
...
export default function WorkoutPlan() {
  ...
  const { data: history, isLoading: historyLoading, refetch: refetchHistory } = useQuery({
    ...
    queryFn: () => base44.entities.StructuredPlan.filter(
  ...
  const completedPlans = (history || []).filter(p => p.completed_date);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-blue-50 p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-600">Choose a preset, generate with AI, or start your session</p>
        </div>
          ...
          <Card className="border-2 border-green-400 bg-gradient-to-r from-green-50 to-emerald-50">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold text-green-600 uppercase tracking-wide mb-1">Ready to go</p>
                  <p className="font-bold text-gray-900 text-lg">{activePlan.name}</p>
                  <div className="flex gap-2 mt-1">
                    <Badge variant="outline" className="text-xs"><Clock className="h-3 w-3 mr-1" />{activePlan.estimated_duration_minutes || '?'} min</Badge>
                    <Badge variant="outline" className="text-xs">{activePlan.exercises?.length} exercises</Badge>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setActivePlan(null)}>Clear</Button>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        {executing && activePlan && (
          <PlanExecutor
            ...
            onCancel={() => { setExecuting(false); toast.info('Workout cancelled'); }}
          />
        )}
              ...
              {historyLoading ? (
                ...
                <Card className="border-dashed border-2 border-gray-200">
                  <CardContent className="pt-12 pb-12 text-center">
                    ...
                    <p className="text-gray-400 text-sm mt-1">Complete a plan to see your history here</p>
                  </CardContent>
                </Card>
              ) : (
                <div className="space-y-3">
                  <p className="text-sm text-gray-600 font-medium">{completedPlans.length} completed workout{completedPlans.length !== 1 ? 's' : ''}</p>
                        ...
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500">
                              {new Date(plan.completed_date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })}
                            </p>
                            <div className="flex gap-2 mt-2 flex-wrap">
                              {plan.difficulty && <Badge variant="outline" className="text-xs">{plan.difficulty}</Badge>}
                              {plan.goal && <Badge variant="outline" className="text-xs capitalize">{plan.goal.replace('_', ' ')}</Badge>}
                            </div>
                          </div>
                          <div className="text-right text-sm space-y-1 flex-shrink-0">
                            {plan.exercises_completed != null && (
                              <div>
                                <span className="font-bold text-purple-600">{plan.exercises_completed}</span>
                                <span className="text-gray-500 text-xs"> exercises</span>
                              </div>
                            )}
                            {plan.total_reps_completed != null && (
                              <div>
                                <span className="font-bold text-green-600">{plan.total_reps_completed}</span>
                                <span className="text-gray-500 text-xs"> reps</span>
                              </div>
                            )}
                            {plan.duration_seconds && (
                              <div className="text-gray-500 text-xs">{formatDuration(plan.duration_seconds)}</div>
                            )}
                            <button

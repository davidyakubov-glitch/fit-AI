...
// Папка nutrition
// Оставь только эти строки, удали все дубликаты выше или ниже!
...
// Скорее всего, PullToRefresh лежит там же, раз они из одной компании
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-xs">
        ...
        <span className="text-gray-500">{Math.round(value)}{unit}</span>
      </div>
...
export default function Nutrition() {
  ...
  const [calorieGoal,  setCalorieGoal]   = useState(() => parseInt(localStorage.getItem(GOAL_KEY)) || DEFAULT_GOAL);
  ...
  const { data: mealLogs = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.MealLog.filter({ created_by: user.email }, '-date', 200),
    enabled: !!user
  });

  // Workout sessions for burned calories
  ...
  const todayTotals = todayMeals.reduce((acc, log) => ({
  ...
  // Burned calories from today's workouts (estimate: 8 kcal/rep average)
  const todayBurned = sessions.filter(s => {
  ...
  const weeklyData = last7Days.map(date => {
    ...
    const dayMeals = mealLogs.filter(log => { const d = new Date(log.date); return d >= ds && d <= de; });
    const totals = dayMeals.reduce((a, l) => ({
    ...
    return { date: format(date, 'EEE'), ...totals, goal: calorieGoal };
  });

  const handleRefresh = async () => queryClient.invalidateQueries({ queryKey: ['mealLogs'] });
        ...
        <div className="max-w-2xl mx-auto p-4 pb-10 space-y-5">
          ...
          <div className="hidden md:flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-500 text-sm">Track your daily food intake</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setShowGoalModal(true)}>
                <Settings2 className="h-4 w-4 mr-2" /> Set Goal
              </Button>
          ...
          <Tabs defaultValue="today" className="w-full">
            ...
            <TabsContent value="today" className="space-y-4 mt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Button variant="outline" size="sm" onClick={() => setShowGoalModal(true)} className="flex-shrink-0">
                  <Settings2 className="h-4 w-4 mr-1" /> Goal
                </Button>
              </div>
              ...
              <Card>
                <CardContent className="pt-4 pb-4 space-y-3">
                  <div className="text-xs font-bold uppercase tracking-wide text-gray-400 mb-3">Macronutrients</div>
                  <MacroBar label="Protein"       value={todayTotals.protein} max={calorieGoal * 0.075} color="bg-blue-500"   />
                  <MacroBar label="Carbohydrates" value={todayTotals.carbs}   max={calorieGoal * 0.125} color="bg-green-500"  />
                  <MacroBar label="Fat"           value={todayTotals.fat}     max={calorieGoal * 0.033} color="bg-yellow-500" />
                </CardContent>
              </Card>
                  ...
                  <div className="grid grid-cols-4 gap-2 md:hidden">
                    {['breakfast','lunch','dinner','snack'].map(type => (
                      ...
                      >
                        ...
                        <span className="text-[10px] text-gray-500 capitalize mt-1">{type}</span>
                      </button>
                    ))}
                  </div>
                );
            ...
            <TabsContent value="history" className="space-y-4 mt-4">
              <Card>
                <CardHeader><CardTitle className="text-base">Daily Calories — Last 7 Days</CardTitle></CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={weeklyData} barSize={24}>
                      ...
                      <Tooltip />
                      <Bar dataKey="calories" fill="#a855f7" radius={[6,6,0,0]} name="Calories" />
                      <Bar dataKey="goal"     fill="#e5e7eb" radius={[6,6,0,0]} name="Goal" />
                    </BarChart>
                  </ResponsiveContainer>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">Macro Trends</CardTitle></CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader><CardTitle className="text-base">Recent Meals</CardTitle></CardHeader>
                <CardContent>
                  <div className="space-y-2">
                      ...
                      <div key={meal.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                        <div>
                          <div className="flex items-center gap-2 mb-0.5">
                            <span className="text-xs text-gray-400">{format(new Date(meal.date), 'MMM d')}</span>
                            <span className="text-xs font-semibold capitalize text-purple-600">{meal.meal_type}</span>
                          </div>
                          <div className="font-semibold text-sm text-gray-900">{meal.food_name}</div>
                    ...
                    {mealLogs.length === 0 && (
                      <p className="text-center text-gray-400 text-sm py-8">No meals logged yet</p>
                    )}
                  </div>
                </CardContent>
              </Card>
